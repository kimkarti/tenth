...
import com.example.tenthskate.Shippment.Activity.ManageProductActivity;
import com.example.tenthskate.Shippment.Class.Upload;


public class adapterRequest extends RecyclerView.Adapter<adapterRequest.ProductsViewHolder> {
    private Context mContext;
    private List<Upload> mUploads;

    DatabaseReference allProducts = FirebaseDatabase.getInstance().getReference().child("ProductsRequest");
    DatabaseReference categoryProducts = FirebaseDatabase.getInstance().getReference().child("RproductsCategory");


    public adapterRequest(Context context, List<Upload> uploads)
    {
        mContext=context;
        mUploads=uploads;
    }
    ...
    public ProductsViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View v= LayoutInflater.from(mContext).inflate(R.layout.productitemrequest, viewGroup,false);
