<?php 
    ...
    if (isset($_GET['id'])) {
        ...
        Delete('tbl_supply','id='.$_GET['id'].'');

        header("location: manage-supplyReq.php");
        exit;
...
?>
	<?php 
		...
		if (empty($keyword)) {
			$sql_query = "SELECT * FROM tbl_supply ORDER BY id DESC";
		} else {
			$sql_query = "SELECT * FROM tbl_supply WHERE name LIKE ? ORDER BY id DESC";
		}
		...
		if (empty($keyword)) {
			$sql_query = "SELECT * FROM tbl_supply where status='0' ORDER BY id DESC LIMIT ?, ?";
		} else {
			$sql_query = "SELECT * FROM tbl_supply WHERE name LIKE ? ORDER BY id DESC LIMIT ?, ?";
		}
...
<div id="content" class="pmd-content content-area dashboard">
	...
	<div class="container-fluid full-width-container">
		...
		<div class="section"> 
				...
				<div class="pmd-card-body">
					...
					<div class="group-fields clearfix row">
						<div class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
							<div class="lead">MANAGE supply</div>
						</div>
			...
			<?php $function->doPages($offset, 'manage-supplyReq.php', '', $total_records, $keyword); ?>
			</form>
		</div>
							...
							<tbody>
								<tr>
									...
									<td>
									    ...
									    <a href="manage-supplyReq.php?id=<?php echo $data['id'];?>" onclick="return confirm('Are you sure want to permanently delete this supply?')" >
