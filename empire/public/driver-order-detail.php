...
<?php 
	...
	$sql_query = "SELECT * FROM driver WHERE id = ?";
...
<?php 
	...
	if (isset($_POST['submit_order'])) {
		...
		$hasil = Update('driver', $data, "WHERE id = '".$_GET['id']."'");
...
EOF;
	...
	if (isset($_POST['cancel_order'])) {
		...
		$hasil = Update('tbl_driver', $data, "WHERE id = '".$_GET['id']."'");

		if ($hasil > 0) {
		    $succes =<<<EOF
				<script>
				alert('This user has been canceled.');
				window.history.go(-1);
				</script>
EOF;
			echo $succes;
...
<div id="content" class="pmd-content content-area dashboard">
	...
	<div class="container-fluid full-width-container">
		...
		<ol class="breadcrumb text-left">
		  ...
		  <li><a href="drivers.php">Manage driver</a></li>
				...
				<div class="pmd-card-body">
					...
					<div class="group-fields clearfix row">
						<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
							<div class="lead">Driver DetailS</div>
						</div>
...
<script>
	function processClicked(e) {
	    if(!confirm("The Driver status can't be changed after processed unless you cancel it."))e.preventDefault();
	}
</script>

<script>
	function cancelClicked(e) {
	    if(!confirm("Are you sure want to cancel driver user  <?php echo $data['fname']; ?>"))e.preventDefault();
	}
</script>